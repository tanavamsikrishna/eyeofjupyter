...
def get_new_snapshot_loc(ipynbfile_path):
    rel_ipynbfile_path = os.path.relpath(ipynbfile_path, os.getcwd())
    snapshot_base_dir = f"{get_project_root()}/{SNAPSHOTS_DIR}/{rel_ipynbfile_path}/"
    os.makedirs(snapshot_base_dir, exist_ok=True)
    snapshot_base_dir_contents = os.listdir(snapshot_base_dir)
    snapshot_version = 0
        ...
        snapshot_version = (
            max(
                [
                    ...
                    if os.path.isdir(f"{snapshot_base_dir}{e}")
                ]
            )
            + 1
        )
    return f"{snapshot_base_dir}{snapshot_version}/"
